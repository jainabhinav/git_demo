...
object mongodb extends DatasetSpec {
  ...
  def sourceDialog: DatasetDialog = {
    val urlSection = StackLayout()
      ...
      .addElement(CodeBlock(title = "Connection URI")
      ...
      .addElement(
        ColumnsLayout(gap = Some("1rem"))
          ...
          .addColumn(TextBox("Cluter IP Address and Options")
        ...
        ColumnsLayout()
          .addColumn(
            StackLayout(direction = Some("vertical"), gap = Some("1rem"))
              ...
              .addElement(
                StackLayout()
                  .addElement(RadioGroup("Credentials")
                  ...
                  .addElement(
                    Condition()
                      ...
                      .otherwise(ColumnsLayout(gap = Some("1rem"))
      ...
      .addSection(
        ...
        ColumnsLayout(gap = Some("1rem"), height = Some("100%"))
          .addColumn(
                ...
                StackLayout()
                  .addElement(StackItem(grow = Some(1)).addElement(FieldPicker(height = Some("100%"))
                    ...
                    .addField(
                      ...
                      "partitioner")
                    .addField(TextBox("Partition field", placeholder = "_id"), "partitionerOptionsPartitionField")
                    .addField(TextBox("Partition size", placeholder = "64"), "partitionerOptionsPartitionSize")
                    .addField(
                      TextBox("Number of samples per partition", placeholder = "10"),
                      "partitionerOptionsSamplesPerPartition")
                    .addField(TextBox("Minimum no. of Docs for Schema inference", placeholder = "1000"), "sampleSize")
                    .addField(
            ...
            "auto")
          .addColumn(SchemaTable("").bindProperty("schema")))
          ...
          .addColumn(
            ScrollBox().addElement(
              StackLayout(height = Some("100%")).addElement(
                StackItem(grow = Some(1)).addElement(FieldPicker(height = Some("100%"))
                  .addField(
                    TextArea("Description", 2, placeholder = "Dataset description...").withCopilot(CopilotSpec(
                      method = "copilot/describe",
                      methodType = Some("CopilotDescribeDataSourceRequest"),
                  ...
                  .addField(
                    SelectBox("Enable Write journal")
                      ...
                      .addOption("false", "false"),
                    "writeConcernJournal")
                  .addField(TextBox("Write timeout in Milliseconds", placeholder = "0"), "writeConcernWTimeoutMs")))),
            "auto")
          .addColumn(
            StackLayout()
              .addElement(StackLayout(height = Some("80bh"))
    ...
    component.properties.credType match {
      ...
      case "userPwd" ⇒
        if (component.properties.textUsername.getOrElse("").trim.isEmpty) {
          diagnostics += Diagnostic("properties.textUsername", "Username cannot be empty", SeverityLevel.Error)
        } else if (component.properties.textPassword.getOrElse("").trim.isEmpty) {
          diagnostics += Diagnostic("properties.textPassword", "Password cannot be empty", SeverityLevel.Error)
        }
    }
      ...
      val conn_uri = props.credType match {
        case "databricksSecrets" ⇒
          f"""${props.driver.get}://${dbutils.secrets.get(
              ...
              key = "username")}:${dbutils.secrets.get(
              scope = props.credentialScope.get,
              key = "password")}@${props.clusterIpAddress.get}"""
        case "userPwd" ⇒
