...
class SQLStatement extends ComponentSpec {
    ...
    Dialog("SQLStatement")
      .addElement(
        ColumnsLayout(gap = Some("1rem"), height = Some("100%"))
          .addColumn(
            Ports(allowOutputRename = true, allowOutputAddOrDelete = true)
              .editableInput(true), "content"
          )
          .addColumn(
  ...
  def onChange(oldState: Component, newState: Component)(implicit context: WorkflowContext): Component = {
    ...
    val revisedFileTabs = newState.ports.outputs.map { port ⇒
      val tab = currentTabs.find(x => {x.path == port.slug || x.id == port.id})
      val newFileTab = FileTab(port.slug, port.id, "sql", SString(""))
      tab match {
