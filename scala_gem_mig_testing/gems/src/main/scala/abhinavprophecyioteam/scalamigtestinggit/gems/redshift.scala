package io.prophecy.warehousegems.dataset
...
object redshift extends DatasetSpec {
  ...
  case class RedshiftProperties(
      ...
      @Property("forward_spark_s3_credentials", "Specifies the behavior when data or table already exists.")
  ...
  implicit val redshiftPropertiesFormat: Format[RedshiftProperties] = Jsonx.formatCaseClass[RedshiftProperties]
...
//            .addElement(TitleElement(title = "Username and Password"))
            ...
            .addElement(ColumnsLayout(gap = Some("1rem")).addColumn(TextBox("Database Name").bindPlaceholder("database_name").bindProperty("path"), width = "4fr")
              .addColumn(TextBox("Table Name").bindPlaceholder("table_name").bindProperty("tableName"), width = "4fr"))))
    .addSection(
      "PROPERTIES",
              ...
              StackLayout()
                .addElement(StackItem(grow = Some(1)).addElement(FieldPicker(height = Some("100%"))
                  .addField(
                    ...
                    "description",
                    true)
                  .addField(Checkbox("Forward S3 access credentials to databricks"), "forward_spark_s3_credentials")
        ...
        .addColumn(
          ScrollBox().addElement(
            StackLayout()
              ...
              .addElement(Checkbox("Temporary AWS access key id").bindProperty("temporary_aws_access_key_id"))
              .addElement(
              ...
              .addElement(SelectBox("Row distribution style for new table")
                .addOption("EVEN", "EVEN")
                .addOption("KEY", "KEY")
                .addOption("ALL", "ALL")
  ...
  override def validate(component: Component)(implicit context: WorkflowContext): List[Diagnostic] = {
    ...
    if (component.properties.secretPassword.forall(_.parts.isEmpty)) {
      diagnostics += Diagnostic(
        ...
        "Password field cannot be empty [Location]",
        SeverityLevel.Error)
    }
    ...
    if (component.properties.secretUsername.forall(_.parts.isEmpty)) {
      diagnostics += Diagnostic(
        ...
        "Username field cannot be empty [Location]",
        SeverityLevel.Error)
    }
...
//      diagnostics += Diagnostic("properties.schema", "Schema cannot be empty [Properties]", SeverityLevel.Error)
  ...
  object RedshiftSecretMigration extends PropertyMigrationObj[RedshiftProperties] {
  def migrationNumber: Int = 1
  override def up(oldProperties: RedshiftProperties): RedshiftProperties = {
    val newProp =
      oldProperties.copy(
        secretUsername = Some(
          SecretValue(
            List(VaultSecret("Databricks", Some(""), Some("0"), Some(oldProperties.credentialScope), "username")))),
        secretPassword = Some(
          SecretValue(
            List(VaultSecret("Databricks", Some(""), Some("0"), Some(oldProperties.credentialScope), "password")))))
    newProp.copy(credentialScope = "")
  }
}

  registerPropertyEvolution(RedshiftSecretMigration)
}
