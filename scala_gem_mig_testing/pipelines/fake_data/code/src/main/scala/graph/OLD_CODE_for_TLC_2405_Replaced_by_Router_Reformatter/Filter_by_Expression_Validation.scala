...
object Filter_by_Expression_Validation {
  def apply(context: Context): (DataFrame, DataFrame) = {
    val spark = context.spark
    val Config = context.config
    
      val (out0, out1) = in.getFilterDataFrame(
        Config.oLD_CODE_for_TLC_2405_Replaced_by_Router_Reformatter__Filter_by_Expression_Validation_rules
      )
    (out1, out0)
  }
