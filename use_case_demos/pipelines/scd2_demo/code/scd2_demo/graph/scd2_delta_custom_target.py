...
def scd2_delta_custom_target(spark: SparkSession, in0: DataFrame):
    from delta.tables import DeltaTable, DeltaMergeBuilder

    if (
        DeltaTable.isDeltaTable(spark, "dbfs:/Prophecy/abhinav/delta_custom_test/test1")
        and spark._jvm.org.apache.hadoop.fs.FileSystem\
          .get(spark._jsc.hadoopConfiguration())\
          .exists(spark._jvm.org.apache.hadoop.fs.Path("dbfs:/Prophecy/abhinav/delta_custom_test/test1"))
    ):
        updatesDF = in0.withColumn("is_min", lit("1")).withColumn("is_max", lit("1"))
        updateColumns = updatesDF.columns
        existingTable = DeltaTable.forPath(spark, "dbfs:/Prophecy/abhinav/delta_custom_test/test1")
...
"is_max" : "0", "to_timestamp" : "staged_updates.from_timestamp"}
        ...
        in0.write.format("delta").mode("overwrite").save("dbfs:/Prophecy/abhinav/delta_custom_test/test1")
