...
class SchemaTransform(ComponentSpec):
    ...
    def onButtonClick(self, state: Component[SchemaTransformProperties]):
        ...
        return state.bindProperties(dataclasses.replace(state.properties, transformations=_transformations))
    ...
    def csv_to_tf(self, state: Component[SchemaTransformProperties]) -> Component[SchemaTransformProperties]:
        ...
        return state.bindProperties(dataclasses.replace(state.properties, transformations=tfs))
                    ...
                    if len(tf.expression.rawExpression.strip()) == 0:
                        ...
                        for rule in rules:
                            if rule.matches_expression(tf.expression.rawExpression):
                                ...
                                input_schema = component.ports.inputs[0].schema if component.ports.inputs[0].schema is not None else StructType()
                                input_schema = StructType([structField for structField in input_schema.fields if structField.name not in dropped_columns])
                                ...
                                for (param_name, param_diags) in param_errors.items():
                                    for diagnostic in param_diags:
                                        diagnostics.append(dataclasses.replace(diagnostic, path=f"properties.transformations[{idx}].expression.expression"))
                            else:
                                continue
                        if rule_output_name is not None:
                            transformed_columns.append(rule_output_name)
                        else:
                            trimmed_expr = tf.expression.rawExpression.lstrip()
                            first_paran_index = trimmed_expr.find("(")
                            if first_paran_index != -1:
                                func_name = trimmed_expr[:first_paran_index]
                                diagnostics.append(
                                    Diagnostic(f"properties.transformations[{idx}].expression.expression",
                                               f"Rule with the name: `{func_name}` is not defined or broken. Please verify.",
                                               SeverityLevelEnum.Error))
                            else:
                                diagnostics.append(
                                    Diagnostic(f"properties.transformations[{idx}].expression.expression",
                                               f"Rule used in `{tf.expression.rawExpression}` is not defined or broken. Please verify.",
                                               SeverityLevelEnum.Error))
    ...
    def onChange(self, context: WorkflowContext, oldState: Component[SchemaTransformProperties],
            ...
            if isinstance(transformation, self.AddReplaceColumn):
                ...
                for rule in context.functions_context.rules:
                    if rule.matches_expression(transformation.expression.rawExpression):
                        rule_output_name = rule.outputs[0].name
                        input_schema = newState.ports.inputs[0].schema if newState.ports.inputs[0].schema is not None else StructType()
                        input_schema = StructType([structField for structField in input_schema.fields if structField.name not in dropped_columns])
                        param_errors = rule.validate_inputs(input_schema, transformed_columns)
                        for (param_name, param_diags) in param_errors.items():
                        ...
                        continue
                updated_transformation = dataclasses.replace(updated_transformation, param_errors=all_param_errors)
                if rule_output_name is not None:
                    transformed_columns.append(rule_output_name)
        ...
        return newState.bindProperties(dataclasses.replace(newProps, columnsSelector=usedCols, transformations=transformations))
