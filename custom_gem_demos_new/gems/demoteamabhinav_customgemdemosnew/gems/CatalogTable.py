...
class catalogTable(DatasetSpec):
    ...
    class CatalogTableProperties(DatasetProperties):
        ...
        createMinMaxFlags: Optional[bool] = True
        scd2SoftDelete: Optional[bool] = False
        scd2ExpressionToCloseRecord: Optional[str] = None

    def sourceDialog(self) -> DatasetDialog:
        fieldPicker = FieldPicker(height=("100%")) \
            .addField(
            TextArea("Description", 2, placeholder="Dataset description...").withCopilotEnabledDescribeDataSource(),
            "description",
            True
        ).addField(
            SelectBox("Provider")
    ...
    def targetDialog(self) -> DatasetDialog:
        ...
        scd2View = StackLayout() \
            ...
            .addElement(
            StackLayout(height=("100%"))
                .addElement(Checkbox("Create min and max flags column in target").bindProperty("createMinMaxFlags"))
                .addElement(
                Condition()
                    .ifEqual(
                    PropExpr("component.properties.createMinMaxFlags"),
                    BooleanExpr(True),
                )
                    .then(
                    StackLayout(height=("100%")).addElement(
                ...
                Checkbox("Mark records as inactive which are not present in source").bindProperty("scd2SoftDelete"))
        ).addElement(Condition()
            .ifEqual(
            PropExpr("component.properties.scd2SoftDelete"),
            BooleanExpr(True),
        )
            .then(
            TextBox("Spark SQL expression to fill to time column for deleted record with").bindPlaceholder(
                "current_date()").bindProperty("scd2ExpressionToCloseRecord")))
    ...
    def validate(self, context: WorkflowContext, component: Component) -> list:
        ...
        if props.writeMode == "merge_scd2":
            ...
            if props.createMinMaxFlags and isBlank(props.minFlagCol):
                diagnostics.append(
                    Diagnostic("properties.minFlagCol", "minFlagCol cannot be empty", SeverityLevelEnum.Error))
            if props.createMinMaxFlags and isBlank(props.maxFlagCol):
                diagnostics.append(
    ...
    class CatalogTableCode(ComponentCode):
        ...
        def targetApply(self, spark: SparkSession, in0: DataFrame):
            ...
            table_name = f"`{self.props.catalog}`.`{self.props.path}`.`{self.props.tableName}`" if self.props.isCatalogEnabled else f"`{self.props.path}`.`{self.props.tableName}`"
            table_exists = False
            if self.props.writeMode == "merge" or self.props.writeMode == "merge_scd2":
                ...
                try:
                    ...
                    table_exists = False
            if self.props.writeMode == "merge" and table_exists:
                ...
                resMatched.execute()
            elif self.props.writeMode == "merge_scd2" and table_exists:
                from delta.tables import DeltaTable, DeltaMergeBuilder
                ...
                updatesDF: SubstituteDisabled = in0.withColumn(minFlagColumn, lit(flagY)).withColumn(maxFlagColumn,
                                                                                                     lit(flagY)) if (
                    self.props.createMinMaxFlags) else in0
                ...
                if self.props.createMinMaxFlags:
                    ...
                    rowsToUpdate: SubstituteDisabled = existingDF \
                        .join(updatesDF, keyColumns, "left")

                    print("rowsToUpdate: " + str(rowsToUpdate.count()))

                    rowsToClose: SubstituteDisabled = rowsToUpdate.where(
                        updatesDF[fromTimeColumn].isNull() & (existingDF[toTimeColumn].isNull())).select(
                        *[existingDF[val] for val in updateColumns])

                    print("rowsToClose: " + str(rowsToClose.count()))

                    rowsToUpdateNewValues: SubstituteDisabled = rowsToUpdate.where(
                        (existingDF[toTimeColumn].isNull()) & (~updatesDF[fromTimeColumn].isNull()) & (
                            cond
                        )) \
                        .select(*[updatesDF[val] for val in updateColumns])
                    ...
                    stagedUpdatesDF: SubstituteDisabled = rowsToUpdateNewValues \
                        .withColumn("mergeKey", lit(None)) \
                        .union(updatesDF.withColumn("mergeKey", concat(*keyColumns))) \
                        .union(
                        rowsToClose.withColumn(fromTimeColumn, expr(self.props.scd2ExpressionToCloseRecord)).withColumn(
                            toTimeColumn, expr(self.props.scd2ExpressionToCloseRecord)).withColumn(
                            "mergeKey", concat(*keyColumns)))
                    ...
                    existingTable \
                        ...
                        .merge(
                        stagedUpdatesDF.alias("staged_updates"),
                        ((concat(*[existingDF[key] for key in keyColumns]) == stagedUpdatesDF["mergeKey"]) & existingDF[toTimeColumn].isNull())
                    ) \
                        .whenMatchedUpdate(
                        condition=((existingDF[toTimeColumn].isNull()) & updateCond) | (
                                stagedUpdatesDF[toTimeColumn] == stagedUpdatesDF[fromTimeColumn]),
                        set={
                            toTimeColumn: ("staged_updates." + fromTimeColumn)
                        }
                    ).whenNotMatchedInsertAll() \
